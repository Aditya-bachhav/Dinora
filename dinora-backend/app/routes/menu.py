...
def get_menu_category(category_slug: str, restaurant_id: int | None = None, db: Session = Depends(get_db)):
    # restaurant_id must be resolved here too: once slugs are scoped per
    # restaurant (see migration 0006) rather than globally unique, the same
    # slug can legitimately belong to two different restaurants — an
    # unscoped lookup would nondeterministically return whichever one
    # happens to match first, which is a real cross-tenant data leak.
    rid = resolve_guest_restaurant_id(db, restaurant_id)
    category = menu_service.get_category_by_slug(db, category_slug, restaurant_id=rid)
    items = (
        db.query(MenuItem)
...
def get_menu_item(item_slug: str, restaurant_id: int | None = None, db: Session = Depends(get_db)):
    rid = resolve_guest_restaurant_id(db, restaurant_id)
    item = menu_service.get_item_by_slug(db, item_slug, restaurant_id=rid)

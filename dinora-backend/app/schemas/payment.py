...
class PaymentSettingsUpdate(BaseModel):
    """
    Admin sets their own restaurant's Razorpay credentials. Both are
    required together — a key_id with no secret (or vice versa) is
    useless, so the endpoint rejects partial updates rather than silently
    leaving a half-configured, unusable credential pair.
    """
    razorpay_key_id: str

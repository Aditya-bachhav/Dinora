...
def create_category(db: Session, restaurant_id: int, name: str, slug: str | None = None) -> Category:
    ...
    # Scoped to restaurant_id: slugs only need to be unique WITHIN a
    # restaurant. An unscoped check here previously meant Restaurant B
    # could never create their own "Desserts" category once Restaurant A
    # had one — see migration 0006.
    if db.query(Category).filter(Category.slug == slug, Category.restaurant_id == restaurant_id).first():
...
def create_menu_item(
    ...
    *,
    ...
    slug = (slug or name).strip().lower().replace(" ", "-")
    # Scoped to restaurant_id — see create_category's comment above.
    if db.query(MenuItem).filter(MenuItem.slug == slug, MenuItem.restaurant_id == restaurant_id).first():

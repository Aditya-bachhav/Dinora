...
def create_table(db: Session, restaurant_id: int, number: int) -> Table:
    ...
    try:
        ...
        # The check above is racy (two near-simultaneous requests can both
        # pass it before either commits) — the unique constraint on
        # (restaurant_id, number) is the real guard. Translate that into
        # the same clean 409 rather than letting a raw IntegrityError surface.
        db.rollback()
        raise HTTPException(status_code=409, detail="A table with this number already exists") from None
    db.refresh(table)
    return table
